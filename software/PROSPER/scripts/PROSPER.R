...
option_list = list(
  ...
  make_option("--PATH_LD", action="store", default=NA, type='character',
              help="Path to LD information [required]"),
  ...
  make_option("--ite", action="store", default=NA, type='character',
              help="MCCV iteration [default: %default]"),
...
############
## Step 1.2. Load optimal tuning parameter from the single-ancestry analysis

delta <- numeric(length = M)
lambda <- numeric(length = M)
for (l in 1:M){
  tmp <- fread2(lassosum_param_path[l])
  delta[l] <- tmp$delta # tmp$delta0
  lambda[l] <- tmp$lambda # tmp$delta0
  rm(list=c("tmp"))
}
...
#   chr <- allchrom[j]
#   prs <- bigreadr::fread2(paste0(opt$PATH_out,"/tmp/PRS_in_all_settings_bychrom/prs_chr",chr,".txt"))
#   return(prs)
# }
# registerDoMC(1)
score = NULL
...
param <- bigreadr::fread2(paste0(opt$PATH_out,"/tmp/PRS_in_all_settings_bychrom/param_chr",allchrom[1],".txt"))
...
fwrite2(score, paste0(opt$PATH_out,"/before_ensemble/score_file",ite,".txt"), col.names = T, sep="\t", nThread=NCORES)
fwrite2(param, paste0(opt$PATH_out,"/before_ensemble/score_param",ite,".txt"), col.names = T, sep="\t", nThread=NCORES)

# if ( opt$verbose >= 1 ) cat(paste0("PRSs in all tuning parameter settings are saved in ", opt$PATH_out,"/before_ensemble/score_file",ite,".txt \n"))
# if ( opt$verbose >= 1 ) cat(paste0("Their corresponding tuning parameter settings are saved in ", opt$PATH_out,"/before_ensemble/score_param",ite,".txt \n"))
