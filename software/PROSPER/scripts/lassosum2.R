...
Ll <- opt$Ll
Ld <- opt$Ld
lambda_min_ratio <- opt$lambda.min.ratio
...
for(mmm in 1:M){
  ...
  lambdapath <- l_path(summmax = summ_max0, nlambda = Ll, lambda_min_ratio = lambda_min_ratio)
  ...
  for (j in 1:length(allchrom)){
    ...
    # load(paste0(opt$PATH_LD,ethnic,"/LD_PROSPER/chr",chr,"_LD.RData"))
