...
sumstats <- bigreadr::fread2(opt$summary)
...
#   }
# }
# if (!tt) N_str = n_eff = as.numeric(opt$N)
N_str = n_eff = as.numeric(opt$N)
...
# # Format files
# paste0(opt$outPath, prefix_file, "_chr", 1:22, ".dbslmm.txt") %>% paste(., collapse = " ") %>%
