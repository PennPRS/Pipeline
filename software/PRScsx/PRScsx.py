...
def main():
    ...
    for chrom in param_dict['chrom']:
        ...
        blk_size = {}
        for pp in range(n_pop):
            ref_dir_pop = os.path.join(param_dict['ref_dir'], param_dict['pop'][pp])
            ld_blk[pp], blk_size[pp] = parse_genet.parse_ldblk(ref_dir_pop, sst_dict[pp], param_dict['pop'][pp], int(chrom), ref)
