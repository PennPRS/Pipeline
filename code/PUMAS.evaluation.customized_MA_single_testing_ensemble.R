...
get_omnibus_weights_new <- function(methods){
  ...
  for (m in 1:length(methods)){
    ...
    r2.ent[,m] = as.numeric(bigreadr::fread2(paste0(output_path,trait_name,".",method,".testing.txt"))[,1])
  }
  w.ent = log( r2.ent )
  for (kk in 1:k){
...
omnibus_prs <- function(prs_methods){
  ...
  for (method in prs_methods){
    ...
    load(paste0(optimal_params_path, method, '/', trait_name, '.', method, '.optimal.indx.RData'))
  ...
  for (j in 1:k) {
    ...
    snp.w <- bigreadr::fread2(paste0(weight_path,trait_name,".",prs_methods[1],".ite",j,".txt"),select=best_param[1])
    for (i in 2:length(prs_methods)){
      snp.w <- cbind(snp.w, bigreadr::fread2(paste0(weight_path,trait_name,".",prs_methods[i],".ite",j,".txt"),select=best_param[i]))
    }
  ...
  for (j in 1:k){
    ...
    pumas.tmp.new <- get_omnibus_sumR2(XtY.vt=xty$validation_test, weights=snp.w, prs.weights=pumas.weights.new[j,], var.Y=stats.pumas$var.Y, N.vt=stats.pumas$N.vt, X.ref=geno.ref)
  ...
  colnames(pumas.weights.new) <- prs_methods
  ...
  write.table(pumas.cor2, paste0(output_path,trait_name,'.',paste0(prs_methods,collapse = '.'),".omnibus.r2.txt"),col.names = T,row.names=F,quote=F,sep="\t")
  ...
  write.table(pumas.cor2.new, paste0(output_path,trait_name,'.',paste0(prs_methods,collapse = '.'),".omnibus.r2.alternative.txt"),col.names = T,row.names=F,quote=F,sep="\t")
}
...
# # Single PRS method r2 on tuning data already obtained using the single_prs() function in a previous step.
