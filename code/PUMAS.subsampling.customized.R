...
option_list = list(
  ...
  make_option("--ld_file", action = "store", default = NA, type = "character"),
  make_option("--rs_file", action = "store", default = NA, type = "character"),
...
PUMAS.II.FUN.I <- function(FunII.GWAS, FunII.Nt, FunII.LD, FunII.LD.block, trait_name, k, chr){
  ...
  for (i in 1:k){
    ...
    print(paste0('Subsampling completed for ite', i,'.'))
...
main <- function(){
  ...
  load(paste0(ld_file))
  load(paste0(rs_file))
  print(paste0('LD and rs files loaded.'))
  # match GWAS SNPs with LD reference
  matched_data <- match_gwas_LD(gwas=gwas,LD=LD_ref,rs=rs_ref,bp=NULL)
  rm(LD_ref)
  rm(rs_ref)
  rm(gwas)
  fwrite(matched_data$gwas_matched,paste0(output_path,trait_name,".gwas_matched.txt"),col.names=T,row.names=F,sep="\t",quote=F)  
  print(paste0('Matching GWAS and LD data completed.'))
